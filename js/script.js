...
function openStasiModal(id) {
    ...
    if (!data) {
        showToast('Sejarah untuk stasi ini belum tersedia.', 'error');
        return;
    }
    ...
    const chairmenList = data.chairmen.map((name, i) =>
        `<li class="flex items-center gap-2 py-1.5 border-b text-sm text-gray-700" style="border-color:rgba(15,31,75,0.06)">
            <span class="w-5 h-5 rounded flex-shrink-0 flex items-center justify-center text-xs font-bold" style="background:rgba(33,72,192,0.08);color:#2148c0">${i+1}</span>
            ${name}
         </li>`
    ).join('');
    ...
    let imageHtml = '';
    if (data.image) {
        imageHtml = `<img src="${data.image}" alt="${data.title}" class="w-full object-cover rounded-2xl shadow-md mb-7" style="max-height:280px">`;
    } else {
        imageHtml = `
        <div class="w-full rounded-2xl mb-7 flex flex-col items-center justify-center" style="height:200px;background:linear-gradient(135deg,rgba(33,72,192,0.05),rgba(33,72,192,0.02));border:2px dashed rgba(33,72,192,0.2)">
            <span style="font-size:3rem">⛪</span>
            <span class="font-bold mt-2" style="color:#2148c0">Gereja Dalam Pembangunan</span>
            <span class="text-sm mt-1" style="color:#6b7280">Lokasi beribadah sementara di rumah umat</span>
        </div>`;
    }

    content.innerHTML = `
        <div class="pr-10">
            <div class="inline-block text-xs font-bold uppercase tracking-widest px-3 py-1 rounded-full mb-3" style="background:rgba(33,72,192,0.1);color:#2148c0">Pelindung: ${data.patron}</div>
            <h2 class="text-2xl md:text-3xl font-bold mb-6 leading-tight" style="color:#0f1f4b">${data.title}</h2>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8">
            <div class="md:col-span-2">
                <div class="flex items-center gap-2 mb-3">
                    <div class="w-1 h-5 rounded-full" style="background:linear-gradient(180deg,#2148c0,#0f1f4b)"></div>
                    <h4 class="font-bold" style="color:#0f1f4b">Sejarah Singkat</h4>
                </div>
                <div class="text-gray-600 leading-relaxed text-sm" style="text-align:justify">${data.history}</div>
            </div>
            <div>
                <div class="sticky top-4 p-5 rounded-2xl" style="background:rgba(33,72,192,0.04);border:1px solid rgba(33,72,192,0.1)">
                    <div class="flex items-center gap-2 mb-3">
                        <div class="w-1 h-5 rounded-full" style="background:linear-gradient(180deg,#c9a84c,#e8c970)"></div>
                        <h4 class="font-bold" style="color:#0f1f4b">Ketua Stasi</h4>
                    </div>
